...
- `CULQI_MODE` — `mock` (default; simulated charges, no external calls) or `live` (real Culqi; also requires a real `sk_` secret key).
- `CULQI_PUBLIC_KEY`, `CULQI_SECRET_KEY` — placeholders until the real keys arrive; the provider selector refuses to go live on placeholder keys.
...
**Go-live checklist (only Culqi remains):** paste `pk_`/`sk_` keys into `.env`, set `CULQI_MODE=live`, verify the live `culqiProvider` in `server/payments.ts` against Culqi's current API (it was written from docs, never run against real keys), and implement event verification in the `/api/webhooks/culqi` stub. No frontend changes needed. Calendly single-use links are already verified end-to-end.
...
**Payments → booking (IMPLEMENTED, mock payment until real Culqi keys arrive).** Core product invariant: **a user must pay before they can book.** Payment is Yape's "código de aprobación" (OTP) flow; booking stays on **Calendly** but is gated behind payment. Server modules under `server/`:

- `server/payments.ts` — `PaymentProvider` interface with two impls: `culqi_mock` (deterministic simulation; OTP `000000` → insufficient funds, `111111` → expired, anything else → approved) and `culqi` (real: Yape token with public key → charge with secret key; **written from docs, unverified against real keys**). `getPaymentProvider()` picks by `CULQI_MODE` + key sanity.
...
**API surface** (`server.ts`): `GET /api/health` (reports paymentMode/schedulingMode/supabase), `POST /api/checkout` (validate → charge → insert `payments` row → on approval mint link + insert `bookings` row → return `schedulingUrl`; 402 on rejection, 400 on validation), `POST /api/bookings/sync` (manual booking-sync trigger, see below), `POST /api/webhooks/culqi` (stub — implement verification when live). Request/response types are shared with the frontend via `src/types.ts` (`CheckoutRequest`/`CheckoutResponse`/`HealthResponse`).
